...
object DayalMcGregor {
  ...
  object Algorithm2 extends PlsModel[Model]{
    ...
    def train(X: DenseMatrix[Double], Y: DenseMatrix[Double], A: Int): Model = {
      ...
      var w_a: DenseMatrix[Double] = DenseMatrix.zeros[Double](K, 1)      // (K x 1) - column vector of W
      var p_a: DenseMatrix[Double] = DenseMatrix.zeros[Double](M, 1)      // (K x 1) - column vector of P
      var q_a: DenseMatrix[Double] = DenseMatrix.zeros[Double](M, 1)      // (M x 1) - column vector of Q
      var r_a: DenseMatrix[Double] = DenseMatrix.zeros[Double](K, 1)      // (K x 1) - column vector of R
      var tt: Double = 0.0
...
//        println(XX)
        ...
        if (M == 1) {
          ...
          w_a = XY                                                        // in this case, XY (K x M) is a column vector (K x 1) since M == 1
        } else {
          ...
          q_a = eigenvectors(::, indexOfLargestEigenvalue).toDenseMatrix.t  // find the eigenvector corresponding to the largest eigenvalue; eigenvector is (M x 1)
          w_a = XY * q_a                                                    // compute X-weights; w_a is (K x 1); expression yields a (K x M) * (M x 1) === (K x 1)
        }
//        println("before w_a")
//        println(w_a)

        w_a = w_a / sqrt(scalar(w_a.t * w_a))                             // normalize w_a to unity - the denominator is a scalar
        r_a = w_a                                                         // loop to compute r_a
        for (j <- 0 to (a - 1)) {
          r_a = r_a - ( scalar(P(::, j).toDenseMatrix.t * w_a) * R(::, j) ).toDenseMatrix.t             // (K x 1) - ( (1 x K) * (K x 1) ) * (K x 1) === (K x 1) - scalar * (K x 1)
        }
        tt = scalar(r_a.t * XX * r_a)                                     // compute t't - (1 x 1) that is auto-converted to a scalar
...
//        println(r_a)
        ...
        W(::, a) := w_a.toDenseVector
        P(::, a) := p_a.toDenseVector
        Q(::, a) := q_a.toDenseVector
        R(::, a) := r_a.toDenseVector
      }
